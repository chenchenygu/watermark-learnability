...
for model_name, sd in tqdm(samples_dict.items()):
    ...
    if "kth" not in model_name and "key_len" not in watermark_config:
        continue

    seed = watermark_config.get("seed", 42)
    key_len = watermark_config.get("key_len", 256)
